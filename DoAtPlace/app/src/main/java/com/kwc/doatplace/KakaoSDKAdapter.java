...
public class KakaoSDKAdapter extends KakaoAdapter {
     ...
     * 기본 설정은 KakaoAdapter에 정의되어있으며, 설정 변경이 필요한 경우 상속해서 사용가능
     */
    @Override
    public ISessionConfig getSessionConfig() {
        return new ISessionConfig() {
            ...
            // 로그인시 인증받을 타입을 지정, 지정하지 않을 시 가능한 모든 옵션이 지정
            ...
            // SDK 로그인시 사용되는 WebView에서 pause와 resume시에 Timer를 설정하여 CPU소모 절약
            // true 를 리턴할경우 webview로그인을 사용하는 화면서 모든 webview에 onPause와 onResume
            // 시에 Timer를 설정해 주어야 한다. 지정하지 않을 시 false로 설정
